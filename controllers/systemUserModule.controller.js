...
_this = this
var thisModule = AppConfigModule.MOD_SYSTEM_USER_MODULE;
    ...
    { 
        ...
        else
        {
            try
            {
                ...
                let savedModule = await SystemUserModuleService.saveModule(module);
                
                if(savedModule && savedModule.isAdd === true)
                {
                    const savedModuleId = savedModule._id;

                    let roles = await SystemUserRoleService.getAllRoles();
                    await Promise.all(roles.map(async (role) => {
                        await SystemUserRoleService.createRoleRight(role._id, savedModuleId);
                    }));
                }

                resStatus = 1;
                resMsg = "Successfully saved Module ";
            }
            catch(e)
            {
                resStatus = -1;
    ...
    else if(id && id != "")
    {
        ...
        else
        {
            try
            {
                ...
                fetchedModule = await SystemUserModuleService.findModuleById(id);
    ...
    else
    {
        ...
        else
        {
            try
            {
                ...
                let moduleList = await SystemUserModuleService.getModules(skipVal, limit, searchStr, sortByCol, sortOrder, isActiveStatus);
...
exports.selectModuleList = async function(req, res, next)
{
    ...
    else
    {
        try
        {
            ...
            let moduleList = await SystemUserModuleService.getModulesForSelect(page, limit, searchStr, onlyActiveStatus);
            ...
            try
            {
                ...
                if(canBeDeletedResponse)
                {
                    if(canBeDeletedResponse.status > 0)
                    {
                        ...
                        resStatus = 1;
                        resMsg = 'Module removed successfully';
                    }
                    else
                    {
...
exports.checkModuleNameValidity = async function(req, res, next)
{
    ...
    else
    {
        try
        {
            let module = await SystemUserModuleService.checkModuleNameForDuplication(moduleId, moduleName,moduleCategory);
            if(module)
            {
                resStatus = -1;
