...
_this = this
var thisModule = AppConfigModule.MOD_CONSORTIUM_USER_ROLE;
var thisModulename = AppConfigModuleName.MOD_CONSORTIUM_USER_ROLE;
...
exports.saveRole = async function(req,res)
{
    ...
    var consortiumUser = await AppCommonService.getSystemUserFromRequest(req);
    var consortiumUserId = await AppCommonService.getSystemUserIdFromRequest(req);
    ...
    { 
        var hasAddRights = await AppCommonService.checkSystemUserHasModuleRights(consortiumUser, thisModule, AppConfigModule.RIGHT_ADD);
        var hasEditRights = await AppCommonService.checkSystemUserHasModuleRights(consortiumUser, thisModule, AppConfigModule.RIGHT_EDIT);
        ...
        else
        {
            try
            {
                ...
                if(roleId == "")
                {
                    role.createdBy = consortiumUserId;
                    role.isDeleted = 0;
                }
                else
                {
                ...
                let savedRole = await ConsortiumUserRoleService.saveRole(role);
                
                if(savedRole && savedRole.isAdd === true)
                {
                    const savedRoleId = savedRole._id;
    
                    let modules = await ConsortiumUserModuleService.getAllModules();
                    await Promise.all(modules.map(async (module) => {
                      await ConsortiumUserRoleService.createRoleRight(savedRoleId, module._id);
                    }));
                }
...
exports.getRoleDetails = async function(req, res, next) {
    ...
    {
        var hasRights = await AppCommonService.checkSystemUserHasModuleRights(consortiumUser, thisModule, AppConfigModule.RIGHT_VIEW);
        if(!hasRights)
        {
            resStatus = -1;
            resMsg = AppConfigNotif.ACTION_PERMISSION_DENIED;
        ...
        else
        {
            try
            {
                ...
                fetchedRole = await ConsortiumUserRoleService.findRoleById(id);
    ...
    else
    {
        ...
        else
        {
            try
            {
                ...
                let roleList = await ConsortiumUserRoleService.getRoles(skipVal, limit, searchStr, sortByCol, sortOrder, isActiveStatus);
...
exports.selectRoleList = async function(req, res, next)
{
    ...
    else
    {
        try
        {
            ...
            let roleList = await ConsortiumUserRoleService.getRolesForSelect(page, limit, searchStr, onlyActiveStatus);
...
exports.removeRole = async function(req, res, next){
    ...
    {
        var hasRights = await AppCommonService.checkSystemUserHasModuleRights(consortiumUser, thisModule, AppConfigModule.RIGHT_DELETE);
        if(!hasRights)
        {
            resStatus = -1;
            resMsg = AppConfigNotif.ACTION_PERMISSION_DENIED;
...
exports.changeRoleStatus = async function(req, res, next)
{
    ...
    {
        var hasRights = await AppCommonService.checkSystemUserHasModuleRights(consortiumUser, thisModule, AppConfigModule.RIGHT_EDIT);
        if(!hasRights)
        {
            resStatus = -1;
            resMsg = AppConfigNotif.ACTION_PERMISSION_DENIED;
...
exports.roleRightDependencies = async function(req, res, next)
{
    ...
    else if(roleId && roleId != "")
    {
        try
        {
            ...
            let remainingModules = await ConsortiumUserModuleService.getAllRemainingModules(allocatedModuleIds);
...
exports.checkRoleNameValidity = async function(req, res, next)
{
    ...
    else
    {
        try
        {
            let role = await ConsortiumUserRoleService.checkRoleNameForDuplication(roleId, roleName);
            if(role)
            {
                resStatus = -1;
  ...
  else if(roleId && roleId != "")
  {
    ...
    else
    {
        try
        {
            ...
            const role = await ConsortiumUserRoleService.findRoleById(roleId);
            if(role) {
            resStatus = 1;
            ...
            await Promise.all(moduleIdArr.map(async (modId) => {
                ...
                await ConsortiumUserRoleService.addOrUpdateRoleRight(rightsObj);
            }));
            }
            else {
            resStatus = -1;
    ...
    else if(id && id != "")
    {
        ...
        else
        {
            try
            {
                ...
                let roleConsortiumUsers = await ConsortiumUserService.checkIfConsortiumUserUsesRole(id);

                if(roleConsortiumUsers) {
                    resStatus = -1;
                    resMsg = 'This Role is associated with organization User(s).';
                }
                else {
                    resStatus = 1;
                }
