var ConsortiumUserRole = require('../models/consortiumUserRole.model');
var AppConfig = require('../appconfig');
var ConsortiumUserRoleRight = require('../models/consortiumUserRoleRight.model');
...
exports.getRoles = async function(skipVal, limit, search, sortByCol, sortOrder, status){
    ...
    try {
        var roles = await ConsortiumUserRole.paginate(fetchOptions, paginationOptions);
        return roles;
    } catch (e) {
...
exports.getRolesForSelect = async function(page, limit, searchStr, onlyActiveStatus){
    ...
    try {
        var roles = await ConsortiumUserRole.paginate(fetchOptions, paginationOptions, null);
        return roles;
    } catch (e) {
        throw Error('Error while Paginating ConsortiumUserRole')
    }
}
...
exports.saveRole = async function(role)
{
    ...
    if(role.id && role.id != "")
    {
        try
        {
            modRole = await ConsortiumUserRole.findById(role.id);
        }
        catch(e){
    ...
    let isAdd = false;
    if(!modRole){
        modRole = new ConsortiumUserRole();
    ...
    try{
        ...
        throw Error("And Error occured while updating the ConsortiumUserRole "+ e);
    }
}
...
exports.findRoleById = async function(roleId){
    ...
    try {
       var role = await ConsortiumUserRole.findOne(options)
       return role;
    } catch (e) {
...
exports.getRoleModuleRights = async function(roleId, moduleId){
    ...
    try {
       var roleRight = await ConsortiumUserRoleRight.findOne(options)
       return roleRight;
    } catch (e) {
        throw Error('Error while Fetching role ' + e)
    }
...
{
    var newRoleRight = new ConsortiumUserRoleRight({
        role: roleId,
        module: moduleId,
        view: 0,
        viewAll: 0,
    ...
    try{
        ...
        throw Error("Error while Creating ConsortiumUserRole Right " + e)
    }
}
...
exports.checkRoleNameForDuplication = async function(id, name) {
    ...
    try {
        ...
        throw Error('Error while Fetching ConsortiumUserRole' + e)
    }
}
...
exports.getAllRoleModuleRights = async function(roleId) {
    ...
    try {
       var roleRights = await ConsortiumUserRoleRight.find(options)
            .populate(populateOptions)
...
exports.addOrUpdateRoleRight = async function(rightsObj)
{
  ...
  if(rightsObj.role && rightsObj.role != "" && rightsObj.module && rightsObj.module != "")
  {
    ...
    try
    {
      modRoleRights = await ConsortiumUserRoleRight.findOne(fetchOptions);
    }
    catch(e){
      throw Error("Error occured while Finding the ConsortiumUserRole Rights")
    }

    if(!modRoleRights){
      modRoleRights = new ConsortiumUserRoleRight();
    ...
    try{
      ...
      // return a Error message describing the reason
      throw Error("Error while Updating ConsortiumUserRole Right " + e)
    }
  }
}
...
exports.getAllRoles = async function() {
  ...
  try {
      var roles = await ConsortiumUserRole.find(fetchOptions);
      return roles;
  } catch (e) {
